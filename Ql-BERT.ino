...
This version of the sketch is optimized for the rubber mat (betty crocker) purchased from Dollarama
*/

#include <Servo.h>
#include <NewPing.h>
#define trigPin 10 // trig = speaker
#define echoPin 6 // echo = microphone
...
float theta1InitialAngle = 151.0;          //This is dependent on the environment
float theta1Max = 163.0;          //Should be between 0 and 180 
float theta1Min = 147.0;         //Should be between 0 and 180
float deltaTheta1 = (theta1Max - theta1Min)/(float(numTheta1States)-1.0);         //The change in servo1's angle when an action is performed on it = calc
int s1 = int((theta1InitialAngle - theta1Min)/deltaTheta1);         //This is an integer between zero and numTheta1States-1 used to index the state number of servo1 = calc
...
float theta2InitialAngle = 139.0; //This is dependent on the environment                
float theta2Max = 163.0;  //Need theta2Max and theta2Min between 0 and 180 
float theta2Min = 115.0; //Need theta2Min and theta2Max between 0 and 180 
float deltaTheta2 = (theta2Max - theta2Min)/(float(numTheta2States)-1.0);    //The change in servo2's angle when an action is performed on it : calc
int s2 = int((theta2InitialAngle - theta2Min)/deltaTheta2);                  //This is an integer between zero and numTheta2States-1 used to index the state number of servo2 : calc
...
float getDeltaDistanceRolled()
{
  ...
  if (abs(deltaDistance) < 45.0 || abs(deltaDistance) > 450.0) //don't count noise    //default is < 57.0 || > 230.0
  {         
    deltaDistance = 0.0;
  }
...
const int rollDelay = 200;                   //Default is 200.  allow time for the agent to roll after it sets its physical state
